    ...
    with pub_ready_plots.get_context(
        ...
        for i, (label, campaigns) in enumerate(first_element.items()):
            ...
            if not gap:
                ...
                if torch.isclose(diff, torch.zeros_like(diff), atol=0.04).any():
                    zeros = torch.isclose(diff, torch.zeros_like(diff), atol=0.04).all(dim=1)
...
if __name__ == "__main__":
    ...
    campaign_dict_overall["joint_look"] = []
    campaign_dict_overall["sequential_look"] = []
    campaign_dict_overall["joint_look_EI"] = []
    campaign_dict_overall["sequential_look_EI"] = []

    campaign_dict_overall_frac = {}
    campaign_dict_overall_frac["joint_look"] = []
    campaign_dict_overall_frac["sequential_look"] = []
    campaign_dict_overall_frac["joint_look_EI"] = []
    campaign_dict_overall_frac["sequential_look_EI"] = []
    ...
    for dataset, ylabel in zip(["Denmark", "Deoxyfluorination", "Cernak", "Borylation"], [r"$\phi(f(\hat{\mathbf{x}}; \mathbf{w}), \mathcal{W})$ ($\uparrow$)", r"$\phi(f(\hat{\mathbf{x}}; \mathbf{w}), \mathcal{W})$ ($\uparrow$)", r"$\phi(f(\hat{\mathbf{x}}; \mathbf{w}), \mathcal{W})$ ($\uparrow$)", r"$\phi(f(\hat{\mathbf{x}}; \mathbf{w}), \mathcal{W})$ ($\uparrow$)"]):
        ...
        campaign_dict = {}
        campaign_dict["joint_look"] = []
        campaign_dict["sequential_look"] = []
        campaign_dict["joint_look_EI"] = []
        campaign_dict["sequential_look_EI"] = []

        campaign_labels = ["Joint 2LA-UCB", "Seq 2LA-UCB-PV", "Joint 2LA-EI", "Seq 2LA-EI-PV"]
    ...
    for j, dataset in enumerate(["Denmark", "Deoxyfluorination", "Cernak", "Borylation"]):
        create_plot(first_element = campaign_dicts[j], second_element = campaign_dicts[j + 2], file_name =f"joint_{dataset}{'_enhance' if args.enhanced else ''}.pdf", gap = True, MAX_BUDGET = 100)

    create_plot(first_element = campaign_dict_overall, second_element = campaign_dict_overall_frac, file_name = f"joint{'_enhance' if args.enhanced else ''}.pdf", gap = True, MAX_BUDGET = 100)
