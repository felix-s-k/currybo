...
if __name__ == "__main__":
    ...
    campaign_dict_overall["sequential_single"] = []
    campaign_dict_overall["sequential_look_single"] = []
    campaign_dict_overall["sequential_look_single_EI"] = []

    campaign_dict_overall_frac = {}
    campaign_dict_overall_frac["sequential_look"] = []
    campaign_dict_overall_frac["sequential_single"] = []
    campaign_dict_overall_frac["sequential_look_single"] = []
    campaign_dict_overall_frac["sequential_look_single_EI"] = []
    ...
    for dataset, ylabel in zip(["Denmark", "Deoxyfluorination", "Borylation", "Cernak"], [r"$\phi(f(\hat{\mathbf{x}}; \mathbf{w}), \mathcal{W})$ ($\uparrow$)", r"$\phi(f(\hat{\mathbf{x}}; \mathbf{w}), \mathcal{W})$ ($\uparrow$)", r"$\phi(f(\hat{\mathbf{x}}; \mathbf{w}), \mathcal{W})$ ($\uparrow$)", r"$\phi(f(\hat{\mathbf{x}}; \mathbf{w}), \mathcal{W})$ ($\uparrow$)"]):
        ...
        campaign_dict["sequential_single"] = []
        campaign_dict["sequential_look_single"] = []
        campaign_dict["sequential_look_single_EI"] = []

        campaign_labels = ["Seq 2LA-UCB-PV", "Seq 1LA-UCB-Single", "Seq 2LA-UCB-Single", "Seq 2LA-EI-Single"]
    ...
    for j, dataset in enumerate(["Denmark", "Deoxyfluorination", "Borylation", "Cernak"]):
        create_plot(first_element = campaign_dicts[j], second_element = campaign_dicts[j + 4], file_name =f"single_{dataset}{'_enhance' if args.enhanced else ''}.pdf", gap = True, MAX_BUDGET = 100)

    create_plot(first_element = campaign_dict_overall, second_element = campaign_dict_overall_frac, file_name = f"single{'_enhance' if args.enhanced else ''}.pdf", gap = True, MAX_BUDGET = 100)
