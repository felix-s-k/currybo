...
def run_benchmark(dataset: ChemistryDatasetLoader, num_samples: list, i: int, budget: int, threshold: float):
    ...
    try:
        ...
        if os.path.exists(f"{dataset.dataset_name}_joint_look_EI_{i}{'_enhance' if dataset.enhanced else ''}_frac.pt"):
            campaign.load_from_file(f"{dataset.dataset_name}_joint_look_EI_{i}{'_enhance' if dataset.enhanced else ''}_frac.pt")
        campaign.run_optimization(budget=NUM_BUDGET, num_seeds=1, random_seed=i, save_file=f"{dataset.dataset_name}_joint_look_EI_{i}{'_enhance' if dataset.enhanced else ''}_frac.pt")
        campaign.save(f"{dataset.dataset_name}_joint_look_EI_{i}{'_enhance' if dataset.enhanced else ''}_frac.pt")

        end_time = time.time()
        print(f" OPTIMIZATION JOINT TWO STEP LA DONE, TIME: {end_time - start_time}", flush=True)
