...
def run_benchmark(dataset: ChemistryDatasetLoader, num_samples: list, i: int, budget: int):
    ...
    try:
        ...
        if os.path.exists(f"{dataset.dataset_name}_joint_look_EI_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt"):
            campaign.load_from_file(f"{dataset.dataset_name}_joint_look_EI_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt")
        campaign.run_optimization(budget=NUM_BUDGET, num_seeds=1, random_seed=i, save_file=f"{dataset.dataset_name}_joint_look_EI_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt")
        campaign.save(f"{dataset.dataset_name}_joint_look_EI_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt")
