...
def run_benchmark(dataset: ChemistryDatasetLoader, num_samples: list, i: int, budget: int):
    ...
    try:
        ...
        if os.path.exists(f"{dataset.dataset_name}_sequential_complete_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt"):
            campaign_one.load_from_file(f"{dataset.dataset_name}_sequential_complete_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt")
        campaign_one.run_optimization(budget=NUM_BUDGET, num_seeds=1, random_seed=i, complete_monitoring=True, save_file=f"{dataset.dataset_name}_sequential_complete_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt")
        campaign_one.save(f"{dataset.dataset_name}_sequential_complete_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt")
        ...
        if os.path.exists(f"{dataset.dataset_name}_sequential_look_complete_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt"):
            campaign_two.load_from_file(f"{dataset.dataset_name}_sequential_look_complete_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt")
        campaign_two.run_optimization(budget=NUM_BUDGET, num_seeds=1, random_seed=i, complete_monitoring=True, save_file=f"{dataset.dataset_name}_sequential_look_complete_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt")
        campaign_two.save(f"{dataset.dataset_name}_sequential_look_complete_{i}{'_enhance' if dataset.enhanced else ''}_mean.pt")
