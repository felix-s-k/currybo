...
def dataset_analysis(loader, variable_names, target, enhance, aggregation_function, maximum_split):
    ...
    for split in splits:
        dataset.farthest_sample_split(num_samples=split)
...
def plot_results(score_matrix, label, ylabel, ratio=False):
    ...
    with pub_ready_plots.get_context(
        ...
        fig.savefig(f"dataset_analysis_{label}_frac_fps.pdf")
...
def main():
    ...
    datasets = [
        (CernakLoader, variable_names_cernak, target_cernak, True, Sigmoid(threshold=0.075), [12], r"Scaled generality ($\uparrow$)"),
        (DenmarkLoader, variable_names_denmark, target_denmark, True, Sigmoid(threshold=2.0), [3, 3], r"Scaled generality ($\uparrow$)"),
        (DoyleLoader, variable_names_doyle, target_doyle, True, Sigmoid(threshold=0.9), [10], r"Scaled generality ($\uparrow$)"),
        (BorylationLoader, variable_names_borylation, target_borylation, True, Sigmoid(threshold=0.9), [20], r"Scaled generality ($\uparrow$)"),
        (DeoxyfluorinationLoader, variable_names_deoxyfluorination, target_deoxyfluorination, True, Sigmoid(threshold=0.9), [25], r"Scaled generality ($\uparrow$)"),
        (CernakLoader, variable_names_cernak, target_cernak, False, Sigmoid(threshold=0.075), [12], r"Scaled generality ($\uparrow$)"),
        (DenmarkLoader, variable_names_denmark, target_denmark, False, Sigmoid(threshold=2.0), [3, 3], r"Scaled generality ($\uparrow$)"),
        (DoyleLoader, variable_names_doyle, target_doyle, False, Sigmoid(threshold=0.9), [10], r"Scaled generality ($\uparrow$)"),
        (BorylationLoader, variable_names_borylation, target_borylation, False, Sigmoid(threshold=0.9), [20], r"Scaled generality ($\uparrow$)"),
        (DeoxyfluorinationLoader, variable_names_deoxyfluorination, target_deoxyfluorination, False, Sigmoid(threshold=0.9), [25], r"Scaled generality ($\uparrow$)"),
    ]
    ...
    for dataset_num, (loader, variable_names, target, enhance, aggregation_function, maximum_split, ylabel) in enumerate(datasets):
        ...
        torch.save(score_matrix, f"dataset_analysis_{label}_frac_fps.pt")
