...
class ParametrizedStyblinskiTang(ParametrizedBaseTestProblem):
    """
    Parametrized version of the StyblinskiTang test function.
    """
    _parameter_defaults = {
        "a": 16.0,
        "b": 5.0,
    ...
    def evaluate_true(self, X: Union[Tensor, np.ndarray]) -> Union[Tensor, np.ndarray]:
        ...
        if self.negate:
            result = -(0.5 * (X**4 - a * X**2 + b * X).sum(dim=-1))
        else:
            result = 0.5 * (X**4 - a * X**2 + b * X).sum(dim=-1)
