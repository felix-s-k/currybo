...
class CernakLoader(ChemistryDatasetLoader):
    """
    Dataset loader for the Cernak dataset.
    """
    def __init__(self, variable_names: List[str], target: str, enhance: bool = True, prefix: str = ""):
        super().__init__(dataset_name="Cernak")
        self.model_path = prefix + 'src/currybo/test_functions/chemistry_datasets/Cernak.pkl'
        if enhance:
            self.csv_path = prefix + 'src/currybo/test_functions/chemistry_datasets/Cernak_enhanced.csv'
        else:
            self.csv_path = prefix + 'src/currybo/test_functions/chemistry_datasets/Cernak_cleaned.csv'
