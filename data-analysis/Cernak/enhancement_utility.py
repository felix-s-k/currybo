...
def generate_smiles(smi):
    ...
    input_fp = fpgen.GetFingerprint(Chem.MolFromSmiles(smi))
    fps = [fpgen.GetFingerprint(Chem.MolFromSmiles(smiles)) for smiles in canon_smi_ls]

    canon_smi_ls = [canon_smi_ls[i] for i in range(len(canon_smi_ls)) if DataStructs.TanimotoSimilarity(input_fp, fps[i]) > 0.75]
